...
plot.state <- function(data, var_name = "enrollment") {
 ...
 ggplot(data = plot_state_data, aes(x = year, y = mean, group = division)) +
   ...
   labs(title(paste(var_name, "means plot by division"), x = "Year", y = var_name))
     ##cat(eval(var_name), "means plot by division"))
}
```
...
plot.county = function(data, var_name = "enrollment", State = "AL", top_bottom = "top", num_counties = 5){
  ...
  ggplot(data_filter, aes(x = year, y = get(var_name), color = area_name)) +
    geom_line() +
    geom_point() + 
    labs(title = paste("Statistic", var_name, "across years by county"), x = "Year", y = var_name) + 
    theme_minimal()
}
```
...
EDU_AB = combine_datasets(A, B)
EDU_AB

plot.state(EDU_AB$State_dat)

plot.county(EDU_AB$County_dat, State = "NC", top_bottom = "top", num_counties = 20)
plot.county(EDU_AB$County_dat, State = "SC", top_bottom = "bottom", num_counties = 7)
plot.county(EDU_AB$County_dat)
plot.county(EDU_AB$County_dat, State = "PA", top_bottom = "top", num_counties = 8)
```
...
AA = processing_wrapper(urlPST01A)
BB = processing_wrapper(urlPST01B)
CC = processing_wrapper(urlPST01C)
DD = processing_wrapper(urlPST01D)
...
# df_PST final combo dataset

plot.state(df_PST$State_dat)


plot.county(df_PST$County_dat, State = "CA", top_bottom = "top", num_counties = 15)

plot.county(df_PST$County_dat, State = "TX", top_bottom = "top", num_counties = 4)

plot.county(df_PST$County_dat)

plot.county(df_PST$County_dat, State = "NY", top_bottom = "top", num_counties = 10)
